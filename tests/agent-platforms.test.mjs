...
test("only verified or compatible may be advertised as supported", () => {
  ...
  assert.match(
    validateRegistry(invalid, new Date("2026-09-14T00:00:00Z")).join("\n"),
...
test("rejects nonexistent calendar dates", () => {
  ...
  const errors = validateRegistry(invalid, new Date("2026-09-14T00:00:00Z"))
  assert.ok(errors.some(error => /reviewedAt must be a valid calendar date/.test(error)))
  assert.throws(
    () => evaluatePlatform(invalid.platforms[0], new Date("2026-09-14T00:00:00Z")),
    /valid UTC dates/
  )
})
...
test("current registry and ecosystem lock validate together", () => {
  assert.deepEqual(
    validateRegistry(registry, new Date("2026-09-14T00:00:00Z")),
    []
  )
  assert.equal(gitBlobSha(registryText), lock.sources.platforms.sha)
  assert.deepEqual(
    validateLock(lock, registryText, new Date("2026-09-14T00:00:00Z")),
    []
  )
})
...
test("upstream drift check is deterministic with injected fetch", async () => {
  const responses = new Map([
    ["contents/VERSION", {
      ...
      content: Buffer.from("3.6.0\n").toString("base64")
    }],
    ["product-routes.json", { sha: lock.sources.products.sha }],
    ["releases/latest", { tag_name: "v3.6.0" }],
    ["git/ref/tags/v3.6.0", {
    ...
    [`git/tags/${"a".repeat(40)}`, {
      object: {
        type: "commit",
        sha: "9cb3318f84ff980d1cac41ab0fafbcec525ef6dd"
      }
    }]
  ])
  ...
  responses.set("releases/latest", { tag_name: "v3.7.0" })
...
test("upstream drift detects a retargeted v3.6.0 tag", async () => {
  ...
  assert.deepEqual(result.drift, [{
    ...
    expected: "9cb3318f84ff980d1cac41ab0fafbcec525ef6dd",
