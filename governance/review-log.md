...
## 2026-09-14：v3.6.0 与漂移追踪

- 依据：上游 `v3.6.0` tag 解引用为 `9cb3318f84ff980d1cac41ab0fafbcec525ef6dd`，VERSION blob 为 `40c341bdcdbe83bbbda981fa85368c0e1a63d0c7`；核对版本化 README、SYNC 和 SMOKE。
- 当前版本、来源与多平台未发布草稿边界已校准；48 / 77 / 56 / 63、8 个图片服务和四办公 Agent 状态保持原值。未执行图片生成、API 转换或远端草稿操作。
- 新增机器事实文件、四仓审阅摘要和核心仓 Issue 通知接口。运行 Node 测试、证据校验与真实只读上游漂移检查通过；通知写入使用注入请求测试，无生产写入试验。
- 模板 `verifiedWith: v3.5.0` 和原有平台实测日期保留。后续任务运行依赖四仓 PR 合并及核心仓 Actions 可用，不能据代码完成宣称自动化已上线。
